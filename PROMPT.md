...
The hierarchy levels (groups/units) can be renamed to fit your domain — for example "Modules/Lessons" or "Parts/Steps" — via the `terminology` field in config.json.
...
3. Follow the workflow in the skill guide:
   - Determine the walkthrough type (educational units vs. repository documentation)
...
- Prioritize a system architecture diagram reused across all units with per-file highlights
